...
let checkinput str list = 
  if Pcre.pmatch ~rex:(Pcre.regexp "ocus|Lemma|Qed|Admitted|Theorem|Proposition|Search|Fixpoint|Require|Axiom|Locate|Check|Print|Definition|Inductive|Open|Variable|Notation") str  then true
...
let get_values str list =
  if Pcre.pmatch ~rex:(Pcre.regexp "ocus|Lemma|Qed|Admitted|Theorem|Proposition|Fixpoint|Search|Require|Axiom|Locate|Check|Print|Definition|Inductive|Open|Variable|Notation") str  then [||]
  else
